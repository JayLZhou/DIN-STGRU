...
class SeqHKOEvaluation(object):
    def __init__(self, seq_len, threholds=None):
        if threholds==None:
            ...
            self._thresholds = np.array([5.0, 10.0, 20.0,  30.0, 40.0])
        else:
...
def seq_eva_hss_csi(true_imgs, pred_imgs):
    hko_eva = SeqHKOEvaluation(20)
...
def plot_seq_mse(datas,names,model_names):
    ...
    plt.savefig('mse_seq.png')
    plt.show()

def sequence_mae(true_fold,pred_fold):
    res = [0 for _ in range(10)]
    valid_root_path = 'valid_test.txt'
    # sample_indexes = np.loadtxt(valid_root_path)
    sample_indexes = list(range(1, 4001, 1))
...
def plot_seq_mae(datas,names,model_names):
    ...
    plt.xticks(x)
    plt.xlabel('Leadtime (Minutes)')
    plt.ylabel('Mean Square Error (MAE)')
    plt.savefig('mae_seq.png')
    plt.show()

def sequence_ssim(true_fold,pred_fold):
    res = [0 for _ in range(10)]
    valid_root_path = 'valid_test.txt'
    # sample_indexes = np.loadtxt(valid_root_path)
    sample_indexes = list(range(1, 4001, 1))
    for i in sample_indexes:
        ...
        for t in range(6, 16, 1):
            ...
            pre_img = pixel_to_dBZ(pre_img)
            true_img = pixel_to_dBZ(true_img)
            current_res = compare_ssim(pre_img,true_img)
            sample_res.append(current_res)
        for i in range(len(res)):
            res[i] = res[i]+sample_res[i]
...
def plot_seq_ssim(datas,names,model_names):
    ...
    plt.savefig('ssim_seq.png')
    plt.show()

def sequence_psnr(true_fold,pred_fold):
    res = [0 for _ in range(10)]
    valid_root_path = 'valid_test.txt'
    # sample_indexes = np.loadtxt(valid_root_path)
    sample_indexes = list(range(1, 4001, 1))
    for i in sample_indexes:
        ...
        for t in range(6, 16, 1):
            ...
            pre_img = pixel_to_dBZ(pre_img)
            true_img = pixel_to_dBZ(true_img)
            current_res = compare_psnr(pre_img,true_img,data_range=85)
            sample_res.append(current_res)
        for i in range(len(res)):
            res[i] = res[i]+sample_res[i]
...
def plot_seq_psnr(datas,names,model_names):
...
def plot_seq_hss_or_csi(datas,names,model_names,type):
    ...
    for idx, name in enumerate(names):
        if name == 'CIKM_predrann_test':
            plt.plot(x, np.mean(datas[name],1), color='black')
        else:
            plt.plot(x, np.mean(datas[name],1))
    plt.grid()
    plt.legend(model_names)
    plt.xticks(x)
    plt.xlabel('Leadtime (Minutes)')
    if type == 'HSS':
        plt.ylabel('The Average of Heidk Skill Score (HSS)')
    if type == 'CSI':
        plt.ylabel('The Average of Critical Success Index (CSI)')
    print(type + '.png')
...
def mae_sequence_test(test_model_list,model_names,is_plot=True):
    seq_mae_res = {}
    for model in test_model_list:
        seq_mae = sequence_mae(test_root, os.path.join(evaluate_root, model))
        seq_mae_res[model] = seq_mae

    if is_plot:
        plot_seq_mae(seq_mae_res,test_model_list,model_names)
    return seq_mae_res

def ssim_sequence_test(test_model_list,model_names,is_plot=True):
    seq_ssim_res = {}
    for model in test_model_list:
        seq_ssim = sequence_ssim(test_root, os.path.join(evaluate_root, model))
        seq_ssim_res[model] = seq_ssim

    if is_plot:
        plot_seq_ssim(seq_ssim_res,test_model_list,model_names)
    return seq_ssim_res

def psnr_sequence_test(test_model_list,model_names,is_plot=True):
    seq_psnr_res = {}
    for model in test_model_list:
        seq_psnr = sequence_psnr(test_root, os.path.join(evaluate_root, model))
        seq_psnr_res[model] = seq_psnr

    if is_plot:
        plot_seq_psnr(seq_psnr_res,test_model_list,model_names)
    return seq_psnr_res
if __name__ == '__main__':
    ...
    # mae_sequence_test(test_model_list, model_names)
    # psnr_sequence_test(test_model_list, model_names)
    # ssim_sequence_test(test_model_list, model_names)
